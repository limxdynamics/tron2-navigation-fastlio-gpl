...
void Preprocess::robosense_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  const bool has_xyzirt = has_field("ring") && has_field("timestamp");
  const std::size_t filter_stride = static_cast<std::size_t>(std::max(1, point_filter_num));
  ...
  if (has_xyzirt)
  {
    ...
    for (const auto &point : pl_orig.points)
    {
      ...
      {
        base_timestamp = point.timestamp;
    ...
    for (std::size_t i = 0; i < pl_orig.size(); ++i)
    {
      ...
      if (range_squared <= blind * blind ||
          (max_scan_range > 0.0 && range_squared > max_scan_range * max_scan_range))
        continue;
      ...
      added_pt.normal_y = 0;
      added_pt.normal_z = 0;
      added_pt.curvature = has_base_timestamp
        ? static_cast<float>((point.timestamp - base_timestamp) * 1000.0)
        : 0.0f;
  ...
  // Fallback for a driver built as XYZI. Accurate deskew requires the
  // repository's default XYZIRT build, but a uniform scan period is safer
  // than assigning every point the same timestamp.
  pcl::PointCloud<pcl::PointXYZI> pl_orig;
  pcl::fromROSMsg(*msg, pl_orig);
  if (pl_orig.empty())
    return;
  pl_surf.reserve(pl_orig.size());
  const double scan_period_ms = SCAN_RATE > 0 ? 1000.0 / SCAN_RATE : 100.0;
  const double time_step_ms = pl_orig.size() > 1
    ? scan_period_ms / static_cast<double>(pl_orig.size() - 1) : 0.0;
  for (std::size_t i = 0; i < pl_orig.size(); ++i)
  {
    if (i % filter_stride != 0)
      continue;
    ...
    added_pt.normal_y = 0;
    added_pt.normal_z = 0;
    added_pt.curvature = static_cast<float>(i * time_step_ms);
    pl_surf.push_back(added_pt);
  }
}
